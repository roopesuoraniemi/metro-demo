...
static int metroTransformLoc = -1;
static int disintegrateLoc = -1;
...
void InitMetro(float size) {
  ...
  metroTransformLoc = GetShaderLocation(instancingShader, "metroTransform");
  disintegrateLoc = GetShaderLocation(instancingShader, "disintegrateAmount");
...
void DrawFinnishMetro(Matrix transform, float disintegrateAmount) {
  SetShaderValueMatrix(instancingShader, metroTransformLoc, transform);
  SetShaderValue(instancingShader, disintegrateLoc, &disintegrateAmount, SHADER_UNIFORM_FLOAT);
...
Vector3 GetMetroDoorLocation(int carIndex, bool side, float size, Vector3 metroPosition) {
  ...
  return Vector3Add(door, metroPosition);
}
...
Vector3 GetMetroEndLocation(float size, Vector3 metroPosition) {
  ...
  return Vector3Add(Vector3Add(lastCenter, Vector3Scale(forward, carLength / 2.0f)), metroPosition);
}
...
Vector3 GetMetroInsideLocation(int carIndex, float size, Vector3 metroPosition) {
  ...
  return Vector3Add((Vector3){carCenterX, center.y - 0.2f, carCenterZ}, metroPosition);
}